...
parser.add_argument('--dataset', type=str, default='movielens', help='movielens')
...
parser.add_argument('--model_type', type=str, default='UMI', help='UMI | ..')
...
def get_model(model_type, item_count, batch_size, maxlen):
    if model_type == 'UMI':
        model = Model_UMI(item_count, args.embedding_dim, args.hidden_size, batch_size, args.num_interest, maxlen)
    else:
...
def prepare_train_data(src, target):
    nick_id, user_gender, user_age, user_occup, pos_item_id, negs_item_id = src
    hist_item, hist_mask = target
    return nick_id, user_gender, user_age, user_occup, pos_item_id, negs_item_id, hist_item, hist_mask


def prepare_test_data(src, target):
    nick_id, user_gender, user_age, user_occup, pos_item_id = src
    hist_item, hist_mask = target
    return nick_id, user_gender, user_age, user_occup, pos_item_id, hist_item, hist_mask
...
def evaluate_full(sess, test_data, model, model_path, batch_size, item_cate_map, save=True, coef=None):
    ...
    for src, tgt in test_data:
        nick_id, user_gender, user_age, user_occup, item_id, hist_item, hist_mask = prepare_test_data(src, tgt)
        user_embs = model.output_user(sess, [user_gender, user_age, user_occup, hist_item, hist_mask])
